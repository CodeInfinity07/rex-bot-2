...
export function setupBotIntegration(app: Express) {
  ...
  app.get('/api/jack/spotify/callback', async (req, res) => {
    ...
    try {
      ...
      logger.error(`❌ Spotify token exchange failed: ${JSON.stringify(error.response?.data) || error.message}`);
  ...
  app.get('/api/jack/spotify/search', async (req, res) => {
    ...
    try {
      ...
      logger.error(`❌ Spotify search failed: ${JSON.stringify(error.response?.data) || error.message}`);
